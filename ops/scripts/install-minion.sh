...
salt_repo_server="192.168.3.167"
salt_master_ip="192.168.3.167"
...
install_saltstack_rhel_repository() {
  ...
  if [ $? -eq 0 ]; then
    ...
    if [ "$(echo $DISTRO_VERSION | grep 5.)" != "" ]; then
      DISTRO_MAJOR_VERSION=5
    elif [ "$(echo $DISTRO_VERSION  | grep 6.)" != "" ]; then
      DISTRO_MAJOR_VERSION=6
    elif [ "$(echo $DISTRO_VERSION  | grep 7.)" != "" ]; then
